...
final class EventSource {
     ...
     */
    init(config: Config, log: Logger) {
        esDelegate = EventSourceDelegate(config: config, log: log)
    }
    ...
    struct Config {
        ...
        var headerTransform: HeaderTransform = { $0 }
        ...
        var idleTimeout: TimeInterval = 300.0
         ...
         */
        var connectionErrorHandler: ConnectionErrorHandler = { _ in .proceed }

        /// Create a new configuration with an `EventHandler` and a `URL`
        init(handler: EventHandler, url: URL) {
...
final class EventSourceDelegate: NSObject, URLSessionDataDelegate {
    ...
    init(config: EventSource.Config, log: Logger) {
        self.log = log
        self.config = config
        ...
        super.init()
        self.eventParser.shutdownAction = { [weak self] in self?.stop() }
    }
    ...
    func start() {
        delegateQueue.async { [weak self] in
            ...
            guard self.readyState == .raw
                ...
                self.log.info("start() called on already-started EventSource object. Returning")
                return
            }
    ...
    private func connect() {
        log.info("Starting EventSource client")
    ...
    func urlSession(_ session: URLSession,
        ...
        if let error = error {
            if (error as NSError).code != NSURLErrorCancelled {
                log.info("Connection error: \(error.localizedDescription)")
            ...
            log.info("Connection unexpectedly closed.")
        }
        ...
        log.info("Waiting \(sleep) seconds before reconnecting...")
    ...
    func urlSession(_ session: URLSession,
        ...
        if (200..<300).contains(httpResponse.statusCode) {
            ...
            log.error("Unsuccessful response: \(httpResponse.statusCode). Check request in Proxyman for further details.")
